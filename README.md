...
| 通知 | 飞书 / 钉钉 / 桌面原生 / 微信订阅消息 | 多渠道学习提醒 |
| 调度 | APScheduler | 每日定时提醒任务 |
| 微信小程序 | uni-app (Vue 3) | 跨平台小程序，共享后端 API |
...
│           ├── wx_notify.py        # 微信订阅消息服务
│           └── notifiers/          # 多渠道通知（桌面原生/飞书/钉钉/微信订阅）
├── frontend/
    ├── index.html
    ├── package.json
...
└── miniapp/                        # 微信小程序（uni-app Vue 3）
    ...
    └── src/
        ...
        ├── main.js                 # Vue 入口
        ├── api/
        │   └── client.js           # uni.request 封装（9个 API 模块）
        ├── utils/
        │   └── markdown.js         # Markdown 解析器（结构化输出）
        ├── components/
        │   ├── MarkdownViewer.vue  # 文档渲染组件
        │   ├── VideoPlayer.vue     # 视频播放组件（原生video+外链卡片）
        │   └── StepCard.vue        # 步骤卡片组件
        ├── stores/
        │   └── plan.js             # Pinia 状态管理
        └── pages/
            ├── dashboard/index.vue # 首页仪表盘
            ├── learn/index.vue     # 学习主页面
            ├── wizard/index.vue    # 创建计划向导
            ├── profile/index.vue   # 个人档案
            ├── review/index.vue    # AI 评审结果
...
| POST | `/api/auth/wx-login` | 微信小程序登录（code 换 token） |
| POST | `/api/auth/dev-login` | 开发环境登录（无需凭证） |
...
1. 在 `backend/.env` 中配置 `WX_APPID` 和 `WX_SECRET`
2. 在微信公众平台 > 订阅消息 中创建消息模板，将模板 ID 填入 `WX_TEMPLATE_ID`
3. 修改 `miniapp/src/pages/settings/index.vue` 中的 `wxTemplateId` 为实际模板 ID
...
| `WX_APPID` | 微信小程序 AppID | 小程序功能需要 |
| `WX_SECRET` | 微信小程序 Secret | 小程序功能需要 |
| `WX_TEMPLATE_ID` | 微信订阅消息模板 ID | 否（订阅消息需要） |
...
| 微信订阅消息 | WeChat API + `wx_notify.py` | 微信小程序 | 需配置 WX_APPID/WX_SECRET/模板ID |
